...
// ==================== OBTENER PERFIL ====================
app.get('/api/perfil/completo', authenticateToken, async (req, res) => {
  try {
    // Obtener idUsuario del token JWT (más seguro) o del query
    const userId = req.user?.userId;
    const queryId = req.query.idUsuario;
    
    const idUsuario = userId || queryId;

    if (!idUsuario) {
      return res.status(400).json({ 
        error: 'Falta idUsuario',
        message: 'Se requiere ID de usuario' 
      });
    }

    console.log(`🔍 Buscando perfil para idUsuario: ${idUsuario}`);

    // ------------------------- SQLITE -------------------------
    if (USE_SQLITE) {
      // 1. Buscar en la tabla Perfil
      const perfil = await sqliteGet(
        `SELECT 
          IdPerfil, IdUsuario, NombreCompleto, CorreoElectronico,
          FechaDeNacimiento, Genero, Biografia, Estado,
          datetime(FechaCreacion) as FechaCreacion
         FROM Perfil 
         WHERE IdUsuario = ?`,
        [idUsuario]
      );
      ...
      if (perfil) {
        console.log(`✅ Perfil encontrado para usuario ${idUsuario}`);
        return res.json(perfil);
      }

      // 2. Si no existe perfil, buscar datos básicos del usuario
      const usuario = await sqliteGet(
        `SELECT 
          IdUsuario, Nombre, Correo,
          datetime(FechaRegistro) as MiembroDesde,
          Nivel, Puntos
         FROM Usuarios 
         WHERE IdUsuario = ?`,
        [idUsuario]
      ...
      if (usuario) {
        console.log(`📋 Usuario encontrado, creando perfil básico para ${idUsuario}`);
        
        // Crear estructura de perfil básico
        const perfilBasico = {
          ...
          Biografia: '',
          Estado: 'Activo',
          MiembroDesde: usuario.MiembroDesde || new Date().toISOString(),
          Nivel: usuario.Nivel || 1,
          Puntos: usuario.Puntos || 0
        };

        // Opcional: Crear perfil en la base de datos automáticamente
        try {
          await sqliteRun(
            `INSERT INTO Perfil (IdUsuario, NombreCompleto, CorreoElectronico, Estado)
             VALUES (?, ?, ?, 'Activo')`,
            [idUsuario, perfilBasico.NombreCompleto, perfilBasico.CorreoElectronico]
          );
          console.log(`✅ Perfil creado automáticamente para usuario ${idUsuario}`);
        } catch (insertError) {
          console.log(`⚠️ No se pudo crear perfil automático: ${insertError.message}`);
        }

        return res.json(perfilBasico);
      }

      return res.status(404).json({ 
        error: 'Usuario no encontrado',
        message: `No se encontró usuario con ID ${idUsuario}` 
      });
    }

    // ------------------------- SQL SERVER -------------------------
    const result = await sqlRequestFromParams({ idUsuario })
      .query(`
        SELECT 
          IdPerfil, IdUsuario, NombreCompleto, CorreoElectronico,
          FechaDeNacimiento, Genero, Biografia, Estado,
    ...
    const usuarioResult = await sqlRequestFromParams({ idUsuario })
      .query(`
        SELECT 
          IdUsuario, Nombre, Correo,
          CONVERT(VARCHAR, FechaRegistro, 120) as MiembroDesde,
          Nivel, Puntos
        FROM Usuarios 
        WHERE IdUsuario = @idUsuario
      `);
    ...
    console.error('❌ Error en GET /api/perfil/completo:', err);
    res.status(500).json({ 
